...
from morse.core.morse_time import time_more
...
class Armature(morse.core.actuator.Actuator):
    ...
    **Armatures** are the MORSE generic way to simulate kinematic chains
    ...
    def _exec_traj(self):
        ...
        try:
            if time_more(trajectory["starttime"], t):
                return
            
            if time_more(t, trajectory["starttime"] + trajectory["points"][-1]["time_from_start"]):
            ...
            for p in trajectory["points"]:
                ...
                if "started" in p and time_more(end, t):
                    # currently going to this waypoint: fine!
                    break

                elif "started" not in p and time_more(end, t):
                ...
                elif "started" not in p and time_more(t, end):
